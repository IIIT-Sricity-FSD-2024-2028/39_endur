...
export async function renderSuperuserDashboard() {
    const [users, cycles, params, logs] = await Promise.all([
        Promise.resolve(JSON.parse(localStorage.getItem("systemUsers")) || []),
        Promise.resolve(JSON.parse(localStorage.getItem("systemFeedbackCycles")) || []),
        Promise.resolve(JSON.parse(localStorage.getItem("systemEvalParams")) || []),
        Promise.resolve(JSON.parse(localStorage.getItem("systemLogs")) || [])
    ]);

    // Push to global for audit-viewer
    ...
    safeSet('statActiveParams', params.filter(p => p.status === 'active').length);
    ...
    if (recentLog) {
        recentLog.innerHTML = logs.slice(0, 5).map(l => `
            <tr>
