...
export function createApp() {
  ...
  // T-110, DEC-115. THE OPENAPI DOCUMENT, generated by walking the routers mounted above.
  //
  // LAST, and that is a requirement rather than tidiness: the generator reads `mountedRouters()`
  // and each router's stack, so a router mounted after this one would be missing from the
  // document. It is lazy — the walk happens on the first request, by which time every mount has
  // run — but mounting it here keeps the ordering true even if that ever stops being lazy.
  //
  // NOT IN PRODUCTION, decided at the MOUNT rather than inside a handler. An internet-facing
  // deployment has no reason to publish its own route table, harmless as this one is — and
  // deciding it here means the route does not EXIST in production, so it answers the same 404
  // as any other unknown URL rather than a 403 that would confirm it would have been there.
