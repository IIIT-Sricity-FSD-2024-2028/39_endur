// The permission simulator - the explain path.
// The one property that matters is that it returns the decision the system would ACTUALLY make:
// it is three lines wrapping the resolver precisely so it cannot drift into a second implementation.
// These tests check the wrapper's own job - turning a request target into a resolver target - and that
// the answer still carries the trace that makes it worth asking.
...
describe('POST /authz/simulate', () => {
  ...
  it('allows what the head can really do, and names the grant that decided it', async () => {
    ...
    // Without the deciding grant the answer is an assertion; with it, it is evidence.
  ...
  it('blocks the same power one section across, and says out_of_scope rather than no_grant', async () => {
    // The most useful distinction on the screen: the head HAS this power, it simply does not reach that
    // section. "No rule grants this" would be a lie.
    const res = await withCsrf(founder, 'post', '/api/v1/authz/simulate').send({
      principalUserId: head.userId,
      capability: 'unit.create',
    ...
    // The rejected candidate is what makes the "what would have to change" answer renderable.
  ...
  it('accepts an explicit "at", and means NOW when it is left out', async () => {
    // The optional time field is spread in rather than passed as an explicit undefined, which under the
    // project's strict settings is not the same thing. Both shapes are sent, because only one used to compile.
    ...
    // The seeded grants have no end date, so a week out is the same answer: the point is that the route
    // ANSWERS both, not that the two differ.
  ...
  it('refuses a capability that does not exist instead of quietly reporting no_grant', async () => {
    // Before the request was validated against the catalogue, a typo resolved to "no grant", which the
    // simulator renders as "no rule grants this" - a real-looking answer to a question nobody understood.
    const res = await withCsrf(founder, 'post', '/api/v1/authz/simulate').send({
      principalUserId: head.userId,
    ...
    // The ordinary validation error, because the check belongs in the request schema rather than the service.
  ...
  it('is guarded by simulator.run, which a level-3 role does not hold', async () => {
    // Seeded to the top two levels only: the trace describes the organisation's whole permission structure.
