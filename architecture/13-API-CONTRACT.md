...
|---|---|---|---|
| **Console** | `/api/v1/*` | Staff JWT | The bulk of the API |
...
| GET | `/me` | — | Session, org, labels, **and the caller's capability set** |
...
`/api/v1/keys` (`apikey.*`), `/api/v1/billing` (`billing.*`), `/api/v1/analysis`
(`analysis.read`). Specified in `45-FEATURE-public-api.md` and `16`.
