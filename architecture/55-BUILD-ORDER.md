...
| **T-108** ✅ | A | — | **A plan that runs out actually runs out — `DEC-113`.** Owner report: *"on plan expiration nothing happens for the client, they are able to continue to use the features granted by the plan, and there is no option to change the plan on expiration."* **Two causes:** `applyExpiredDowngrade` returned early unless a downgrade had been *scheduled*, and `requireEntitlement` selected `tier` alone and never looked at the date — so even a correct row would not have helped an org that never opens `/app/plan`. `16` §7's *"expiry moves the org to Bronze"* had been contradicting `DEC-080`'s *"nothing expires"* in the docs the whole time. One `effectiveTier()` read by the gate, by `readBilling` and by the estate list; `subscriptions.lapsed_from` so the page can name what was lost; `kind: 'lapse'` beside `expiry`; `pricedFrom` so a rejoin pays ₹999 and not ₹900. `<PlanNoticeBanner>` in `<AppShell>` — the end date was already on `/app/plan` and the owner still met *"nothing happens"* | `16` §7d, `49`, `70`, `10` §, `24` §2, `DEC-113` |
| **T-109** ✅ | A | T-059 | **Support access — every workflow reachable by the superuser. `DEC-114`, supersedes one row of `19` §14.** An operator opens a customer's own console for an hour: a typed reason the customer reads verbatim on every page, a synthetic member under the **operator's own name**, and powers minted as **grants** rather than a bypass — `authz/collect.ts`'s no-person-node branch, `resolve()` unchanged. `SUPPORT_DENIED_CAPABILITIES` (feedback content, personal content, org.delete + billing.update) resolves as `deny` at `all` scope, so **INV-004 is what enforces INV-011 here** and the refusal is `explicit_deny` with the grant that decided it. `<SupportBanner>` above the top bar renders for **both** audiences — the first draft read the caller's own session and was therefore visible only to the operator it was disclosing. `platform/db.ts` was **not** widened; the two tenant writes live in `db/support.ts`. 15 tests | `19` §15, `13` § Platform, `24`, `70`, `DEC-114` |
