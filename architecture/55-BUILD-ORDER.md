...
| 5 | T-082 | C | **T-081 ✅** | `/app/analysis` — the 402-vs-403 screen, now demonstrable (`T-088`) and now **reachable** (`D-033`). Backend is live and 36 tests deep; this is the page | `43`, `16` §3 |
| 6 | T-083 | A | — | Improve-loop backend | `44` |
| 7 | T-084 | B | T-083 | `/app/reflect` | `44` |
| — | T-085 | B | each of the above | **Un-disable the sidebar, one item at a time.** Still the *last edit of each task*, never a task of its own. Roles ✅ and Inbox ✅ done at `T-080`; Analysis and Reflect remain. **The Roles miss is the argument for the rule:** its page shipped 24 Aug and the item stayed greyed for a day, because nothing asserted the *positive* direction. `Sidebar.test.tsx` now does, per built page | `20` §2 |
