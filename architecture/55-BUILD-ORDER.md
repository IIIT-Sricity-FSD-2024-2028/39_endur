...
| — | T-085 | B | each of the above | **Un-disable the sidebar, one item at a time.** Still the *last edit of each task*, never a task of its own. Roles ✅ and Inbox ✅ done at `T-080`, **Analysis ✅ at `T-082`**; Reflect remains. **The Roles miss is the argument for the rule:** its page shipped 24 Aug and the item stayed greyed for a day, because nothing asserted the *positive* direction. `Sidebar.test.tsx` now does, per built page | `20` §2 |
